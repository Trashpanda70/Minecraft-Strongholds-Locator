...
public class RingCalculator
{
	...
	private static double trig(int f, double angle, int hyp) {
		...
		switch (f) {
		case 0:
			return round(Math.abs(Math.sin(d)) * hyp);
		case 1:
			return round(Math.abs(Math.cos(d)) * hyp);
		default:
	...
	public String[][] CoordsToArray(LinkedList<Coords> list) {
		...
		for (Coords c : list) {
			...
			case POSPOS:
				pp[ppCount++] = c.toString();
				break;
			case POSNEG:
				pn[pnCount++] = c.toString();
				break;
			case NEGPOS:
				np[npCount++] = c.toString();
				break;
			case NEGNEG:
				nn[nnCount++] = c.toString();
				break;
			default:
				throw new NullPointerException("Coords with no Quadrant cannot be added");
			}
		}
